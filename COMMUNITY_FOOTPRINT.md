...
## 2026-08-27
- [pypa/cibuildwheel#2977](https://github.com/pypa/cibuildwheel/pull/2977) — docs-only fix for four dead external links: CircleCI's open-source and configuration-reference pages, plus the CPython Android and iOS testbed README paths. The four replacement URLs were checked live from exact head `0ac3d2da`; the PR is OPEN/MERGEABLE with no reviewer requested, four successful contexts, one neutral check and one pending context at this audit. Not merged or accepted yet.
- [tox-dev/tox#4042](https://github.com/tox-dev/tox/pull/4042) — docs-only dead-link fix: `docs/explanation.rst` now points virtualenv discovery at `https://virtualenv.pypa.io/en/stable/how-to/usage.html#select-a-python-version` instead of the removed `https://virtualenv.pypa.io/en/latest/user_guide.html#python-discovery` page (old URL 404, replacement 200); added `docs/changelog/4042.doc.rst`. Exact head `4ef082c9`; the PR reports `tox run -e docs`, `tox run -e fix`, and targeted pre-commit hooks passing. Live at audit: OPEN/MERGEABLE, reviewer `rahuldevikar` requested, 0 reviews/comments, 20 of 30 contexts successful, 2 `tox env type` failures attributed in the PR to unrelated dependency drift, and 8 still in progress. Not merged or accepted yet.
