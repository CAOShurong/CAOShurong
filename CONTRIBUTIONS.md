...
- **[astropy/astropy #20297](https://github.com/astropy/astropy/issues/20297#issuecomment-5606900901)** — comprehensive architectural root cause analysis and fix for `TypeError: list indices must be integers or slices, not NoneType` in `aggregate_downsample(ts["time", "a"], ...)`: identified that `Table.__getitem__` drops `primary_key` when slicing columns, `TimeSeries.__init__` bypasses `add_index("time")` when pre-existing indices are present in `data`, and `TableLoc._get_index_id_and_item()` passes `None` to `TableIndices` (which triggers `list.__getitem__(None)`). Proposed a robust 3-layer solution across `Table.__getitem__`, `TimeSeries.add_column()`, and `TableLoc`.
- **[fastapi/fastapi #16337](https://github.com/fastapi/fastapi/discussions/16337#discussioncomment-18371950)** — root-cause analysis and verified solution for `Annotated[AsyncIterator[T], ...]` return annotations failing route registration: traced the `_STREAM_ORIGINS` rejection of `typing.Annotated` in `get_stream_item_type()`, demonstrated the OpenAPI metadata tradeoff where `response_model=int` leaves `itemSchema: {}` untyped, and proved native unwrapping produces clean JSONL chunk schemas. Branch `fix-annotated-stream-item` prepared.
