...
- [anchore/syft#5241](https://github.com/anchore/syft/issues/5241#issuecomment-5458594889) — P4 独立验证：pnpm cataloger 遇到 `devEngines.runtime` 的 `variations` resolution 时，因 `Resolution map[string]string` 无法解码 sequence 而触发 `*yaml.TypeError`，导致整个 lockfile 被丢弃（0 packages）。在 main `40179796` 处实测复现；补丁用 `errors.As` 容忍 `*yaml.TypeError` 并保留部分解码结果，复测得到 2 artifacts（`is-odd` + `node runtime:26.8.1`），clean lockfile 仍为 1 artifact。全量 pnpm 测试通过、go build/vet 干净。已披露 AI 辅助。
- [ossf/scorecard#5203](https://github.com/ossf/scorecard/pull/5203#pullrequestreview-5055383719) — P2 验证型 review（针对 `Eljees` 的第三方修复，解决 #5072：`FormatActionlintError()` 在解析 YAML 报错时未包含工作流文件路径，导致多 workflow 仓库无法定位是哪个文件出错）。独立验证（no run no claim）：克隆 scorecard 到干净 worktree，checkout PR head `7676c790`。`go build ./...` 与 `go vet ./checks/... ./probes/hasDangerousWorkflowScriptInjection/...` 均 clean。相关测试通过：`TestFormatActionlintError`、新增回归测试 `TestFormatActionlintError_IncludesFilePath`、以及全部 probe 测试。base `main` 上存在两个预存 Windows 失败（`TestIsWorkflowFile` 路径 fixture、`TestCollectGitHubActionsWorkflowPinning` fixture panic），与本 PR 无关。评审中披露 AI 辅助，REVIEW 状态 APPROVED，是诚实的 review 证据，不是接受/维护权/合并权。KPI 更新：external merged **29/100**（16 repos / 15 owners）；authored open **71**（41 repos / 36 owners）；正式 PR-review 贡献 **54**（GitHub 2026-08-01–2026-08-29 统计）。
...
- [ossf/scorecard#5202](https://github.com/ossf/scorecard/pull/5202) — extends
  the Packaging detector to recognize `gh release create/upload/edit` and the
  official Nextcloud App Store release API, while keeping `gh release view` as
  a non-match. Exact head `e7efdc2e`; focused Go tests, `go vet`, and diff
  checks pass locally, and PR Verifier, DCO and Kusari Inspector are green.
  OPEN/MERGEABLE and awaiting maintainer review; the static change does not
  claim a Packaging score without a successful workflow run.
